...
**Goal:** empty Next.js 15 / TS strict / pnpm app, three-branch protection on, Vercel project linked, secrets configured.
**Adds:** `package.json`, `tsconfig.json` (strict + `noUncheckedIndexedAccess` + `exactOptionalPropertyTypes`), `next.config.ts`, `biome.json`, `app/layout.tsx`, `app/page.tsx` (placeholder), `app/globals.css` (imports `tokens.css`), `.github/workflows/ci.yml` (skeleton), `.github/workflows/branch-flow-guard.yml`, PR + issue templates, `.env.example`, `CLAUDE.md` (lists installed Vercel agent skills).
**Tests:** none. CI runs typecheck + lint + build only.
**Verify:** `pnpm typecheck && pnpm build` green; CI green on PR to `develop`; Vercel preview renders the placeholder; direct push to `main`, `staging`, or `develop` rejected by GitHub; `branch-flow-guard` rejects a `feature/*` PR opened against `staging` or `main`.
