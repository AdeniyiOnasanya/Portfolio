...
4. **Reduced motion.** Use `mcp__chrome-devtools-mcp__evaluate_script` to set the media query at runtime: run a script that calls `window.matchMedia('(prefers-reduced-motion: reduce)')` and inspects whether reduced-motion-respecting CSS or JS branches are taking effect. If the slice introduces an animation, also use `mcp__chrome-devtools-mcp__emulate_cpu` plus a script-driven `document.documentElement.style.setProperty` injection to simulate `prefers-reduced-motion: reduce`, then verify no transform or transition runs longer than 1ms via the Performance API. CLAUDE.md hard rule: every animation honours `prefers-reduced-motion`.
